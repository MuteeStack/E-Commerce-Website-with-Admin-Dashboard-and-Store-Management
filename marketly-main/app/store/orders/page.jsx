...
import { getStoreByUserId, getOrdersByStore, getUser, getProduct, updateOrder } from "@/lib/firebaseDb"
...
export default function StoreOrders() {
    ...
    const fetchOrders = async () => {
        ...
        try {
            ...
            const storeOrders = await getOrdersByStore(store.id)

            // Enrich orders with user, address, and product info
            const enrichedOrders = await Promise.all(
                storeOrders.map(async (order) => {
                    const orderUser = await getUser(order.userId)
                    // Enrich order items with product info
                    const enrichedItems = order.orderItems
                        ? await Promise.all(
                            (Array.isArray(order.orderItems) ? order.orderItems : Object.values(order.orderItems)).map(async (item) => {
                                const product = await getProduct(item.productId)
                                return { ...item, product: product || { name: 'Unknown', images: [] } }
                            })
                        )
                        : []
                    return {
                        ...order,
                        user: orderUser || { name: 'Unknown', email: '' },
                                ...
                                {selectedOrder.orderItems.map((item, i) => (
                                    <div key={i} className="flex items-center gap-4 border border-slate-100 shadow rounded p-2">
                                        {item.product?.images?.[0] && (
                                            <img
                                                src={typeof item.product.images[0] === 'object' ? item.product.images[0].src : item.product.images[0]}
